/**
 * invoiceLoyaltyUserEvent.ts
 * by shelby.severin@trevera.com
 *
 * @NScriptName HF | Loyalty | Invoice User Event
    ...
    function afterSubmit(context) {
        ...
        if (context.type == context.UserEventType.CREATE || (context.type == context.UserEventType.EDIT && runtime.getCurrentUser().id == 339)) {
            ...
            if (config.optedIn && Number(createdFrom) > orderCutoff) {
                ...
                /** on creation of an invoice, if the items on the invoice are eligible for points and the user is opted in with a valid tier then create a points entry for them to earn points */
                ...
                if ((0, loyaltyGlobals_1.isCreatedFromSO)(createdFrom)) {
                    ...
                    /** if the invoice is created from a SO and points were redeemed, then update the pending points entry to be 'redeemed' instead of 'pending' */
                    if (redeemedPoints)
