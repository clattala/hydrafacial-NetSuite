define('OrderWizard.Module.PointsRedemption'
  ...
  , function (
    ...
    return WizardModule.extend({
      ...
      events: {
        'blur [data-action="validate-points"]': 'validateEntry',
      ...
      validateEntry: function validateEntry(e) {
        e.preventDefault();
        e.stopPropagation();
        var self    = this;
        var $target = jQuery(e.currentTarget);
        var amt = Number($target.val());
        var amtRounded = Math.floor(Math.abs(Number($target.val())));
        if(amt != amtRounded) $target.val(amtRounded);
      ...
      redeemPoints: function (e) {
        ...
        var points = !options.points ? 0 : Math.floor(Math.abs(Number(options.points))); // normalize and round value.
        console.log(options.points, points, 'this.userPointsConfig[\'availablePoints\']', this.userPointsConfig['availablePoints']);
        if (!options.points || points < 1) {
          this.currentState.errorMessage = Utils.translate('Points are required');
          this.render();
        }
        else if (points > Number(this.userPointsConfig['availablePoints'])) {
          this.currentState.errorMessage = Utils.translate('You cannot redeem more points than your maximum available points.');
          this.render();
        }
        else {
          var discount       = points * (parseFloat(self.configuration.ratio));
          discount           = Math.floor(Math.abs(Number(discount)));
              ...
              if (result.status == "OK") {
                self.cartComponent.addPromotion({
                  ...
                  _.extend(cartOptions, {
                    ...
                    custbody_loyalty_points_red_redeemed: Math.floor(points).toString()
                  })
