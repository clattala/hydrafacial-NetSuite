...
<li class="header-menu-myaccount-overview">
	...
	<a class="header-menu-myaccount-signout-link" href="#" data-touchpoint="logout" name="signout">
		...
		{{#each entries}}
    <li class="header-menu-myaccount-item-level2 line-item-{{name}}" {{#if permission}}data-permissions="{{permission}}"{{/if}} {{#if permissionOperator}}data-permissions-operator="{{permissionOperator}}"{{/if}}>
        {{#if url}}
