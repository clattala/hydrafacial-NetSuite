...
define('Tavanoteam.TT_Pacejet.Pacejet'
  , [
    'Tavanoteam.TT_Pacejet.Pacejet.ServiceController', 'Application', 'Models.Init', 'Profile.Model', 'Pacejet.Model', 'Configuration', 'Utils'
  ]
  , function (
    PacejetServiceController, Application, ModelsInit, ProfileModel, PacejetModel, Configuration, Utils
  ) {
    'use strict';
      ...
      try {
        ...
        if (data.shipmethod) {
          ...
          if (pacejetResponse && pacejetResponse.ratingResultsList && pacejetResponse.ratingResultsList.length > 0) {
            ...
            if (pjShippingMethod) {
              var flatRateAmount                        = parseFloat(ctx.getSessionObject('flatRateAmount'));
              var pjShippingMethodCost                  = pjShippingMethod[pacejetConfig.priceFieldToUse] || 0;
            ...
            // var shippingCost = Math.floor(Math.random() * 100).toString();
            // data.options.custbody_tt_pj_shipping_cost = shippingCost;
          }
        }
      ...
      try {
        ...
        if (pacejetResponse) {
          ...
          if (pacejetResponse && pacejetResponse.ratingResultsList && pacejetResponse.ratingResultsList.length > 0) {
            ...
            if (calculatedWeight < showLtlOver) {
              ret.shipmethods = _.filter(ret.shipmethods, function (shipMethod) {
            ...
            try {
              ...
              if (pcShipMethod) {
                ...
                if (shippingPromotions.length < 1) {
