define('Trevera.PDP.Helpers'
  , [
    ...
    return {
      ...
      handleJumpstartItems: function (CART, LAYOUT, jumpstartItemIds) {
        var self = this;
        CART.on('afterShowContent', function (data) {
          console.log('afterShowContent', data);
          _.each(jumpstartItemIds, function (id) {
            self.checkLineQuantityMatrixFromParent(CART, LAYOUT, Number(id));
          })
        })
        CART.on('afterAddLine', function (line) {
          if(jumpstartItemIds.indexOf(line.line.item.internalid.toString()) > -1) {
            console.log('afterAddLine', line);
