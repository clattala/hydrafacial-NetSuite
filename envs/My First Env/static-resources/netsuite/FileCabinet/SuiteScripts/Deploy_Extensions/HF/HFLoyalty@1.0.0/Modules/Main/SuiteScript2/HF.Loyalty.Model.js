...
define(['N/https', 'N/log', 'N/query', 'N/record', 'N/runtime', 'N/search'], function (https, log, query, record, runtime, search) {
    ...
    return {
      getUserPoints: function getUserPoints(request) {
        ...
        var customerLookup = search.lookupFields({type: record.Type.CUSTOMER, id: currentUser, columns: ['custentity_hf_loyalty_optin', 'custentity_hf_current_loyalty_tier']})
        ...
        if (isOptedIn) {
          ...
          if(Number(currentTier[0].value) > 0) {
            var tierConfig = LOYALTY_CONFIG[currentTier[0].value];
            log.debug('tierConfig', tierConfig);
            if (!tierConfig) {
              log.error('no config found for this tier ' + currentTier, LOYALTY_CONFIG.tiers);
              resp.optedIn = false;
            }
            else {
      ...
      getConfig: function getConfig() {
        ...
        var cachedConfig = sessionObj.get('hf_loyalty_tier');
        if (!cachedConfig) {
          var tierMap        = {};
          ...
          sessionObj.set('hf_loyalty_tier', cachedConfig);
        }
